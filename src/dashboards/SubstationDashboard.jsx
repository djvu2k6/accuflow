...
const SubstationDashboard = () => {
    ...
    const savingsClosing = parseFloat(bankData.savings_opening || 0) + parseFloat(bankData.savings_dep || 0) - parseFloat(bankData.savings_with || 0);
    const fdClosing      = parseFloat(bankData.fd_opening || 0) + parseFloat(bankData.fd_dep || 0) - parseFloat(bankData.fd_with || 0);
    ...
    return (
        <div className="min-h-screen bg-[#f1f5f9] p-4 md:p-8 space-y-10 font-sans pb-20">
            ...
            <div className="border-b-2 border-slate-300 pb-6">
                ...
                <p className="text-slate-500 font-bold text-sm mt-1 uppercase tracking-widest">Branch Data Entry & Reporting</p>
            </div>
            ...
            <div className="max-w-5xl mx-auto bg-white rounded-[2.5rem] border-2 border-slate-900 shadow-xl overflow-hidden">
                <div className="p-6 bg-slate-900 text-white font-black uppercase tracking-widest text-sm">
                    Branch Bank Statement Reconciliation
                </div>
                <div className="p-8 grid grid-cols-1 md:grid-cols-2 gap-10">
                    ...
                    <div className="space-y-4">
                        ...
                        <div className="space-y-1.5">
                            ...
                            <input type="number" className="w-full p-3 border-2 border-slate-200 rounded-xl font-bold text-sm outline-none focus:border-blue-400"
                                placeholder="0" onChange={e => setBankData({ ...bankData, savings_opening: e.target.value })} />
                        </div>
                        <div className="space-y-1.5">
                            <label className="text-[10px] font-black text-slate-400 uppercase">Deposit</label>
                            <input type="number" className="w-full p-3 border-2 border-slate-200 rounded-xl font-bold text-sm outline-none focus:border-blue-400"
                                placeholder="0" onChange={e => setBankData({ ...bankData, savings_dep: e.target.value })} />
                        </div>
                        <div className="space-y-1.5">
                            <label className="text-[10px] font-black text-slate-400 uppercase">Withdrawal</label>
                            <input type="number" className="w-full p-3 border-2 border-slate-200 rounded-xl font-bold text-sm outline-none focus:border-blue-400"
                                placeholder="0" onChange={e => setBankData({ ...bankData, savings_with: e.target.value })} />
                        </div>
                    ...
                    <div className="space-y-4">
                        ...
                        <div className="space-y-1.5">
                            ...
                            <input type="number" className="w-full p-3 border-2 border-slate-200 rounded-xl font-bold text-sm outline-none focus:border-indigo-400"
                                placeholder="0" onChange={e => setBankData({ ...bankData, fd_opening: e.target.value })} />
                        </div>
                        <div className="space-y-1.5">
                            <label className="text-[10px] font-black text-slate-400 uppercase">Deposit</label>
                            <input type="number" className="w-full p-3 border-2 border-slate-200 rounded-xl font-bold text-sm outline-none focus:border-indigo-400"
                                placeholder="0" onChange={e => setBankData({ ...bankData, fd_dep: e.target.value })} />
                        </div>
                        <div className="space-y-1.5">
                            <label className="text-[10px] font-black text-slate-400 uppercase">Withdrawal</label>
                            <input type="number" className="w-full p-3 border-2 border-slate-200 rounded-xl font-bold text-sm outline-none focus:border-indigo-400"
                                placeholder="0" onChange={e => setBankData({ ...bankData, fd_with: e.target.value })} />
                        </div>
                    ...
                    <button
                        ...
                        disabled={loading}
                        className="col-span-full bg-slate-900 text-white py-4 rounded-xl font-black uppercase text-xs tracking-widest border-2 border-slate-900 hover:bg-blue-800 transition-all disabled:opacity-50">
                        Sync Branch Statement
                    </button>
                </div>
            </div>
            ...
            <div className="max-w-5xl mx-auto bg-white rounded-[2rem] border-2 border-slate-900 overflow-hidden shadow-lg">
                <div className="p-6 bg-slate-900 text-white font-black uppercase tracking-widest text-sm text-center">
                    ...
                    {history.map((item, i) => (
                        <div key={i} className="p-6 flex justify-between items-center hover:bg-slate-50 transition-colors">
                            ...
                            <p className={`text-xl font-black ${item.income > 0 ? 'text-emerald-700' : 'text-rose-600'}`}>
                                {item.income > 0 ? `+ ₹${Number(item.income).toLocaleString()}` : `− ₹${Number(item.expense).toLocaleString()}`}
                            </p>
                        </div>
                    ))}
                </div>
